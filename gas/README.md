...
**先頭が 0 のパスワードは使えない。** Sheets は数字のみの値を数値として扱うため、
`0123` は `123` になってしまう。0 で始めたい場合はセルを書式設定で文字列にするか、
先頭に英字を含める。

**4桁の数字は総当たりに弱い。** 組み合わせは1万通りしかなく、`login` は
公開エンドポイントで試行回数の制限もない。桁数を増やすか英数字を混ぜることを推奨する。
